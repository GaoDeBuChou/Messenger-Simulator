...
extern qqUsersManagerXXX a1;
extern mBlogManagerXXX b1;
extern WeChatUsersManagerXXX c1;
...
int qqGroupXXX::MakemBlogGroup() {
	...
	int newgroupid;
	int i;
	b1.AddGroup(name, Head->person->returnmBlog()->ID(), date);
	newgroupid = b1.Number_of_Groups() + 1000;
	for (i = 1; i < number_of_members; i++) {
		if (GroupMembers[i]->person->returnmBlog() != 0) {
			b1.ingroup(newgroupid, GroupMembers[i]->person->returnmBlog()->ID());
		}
	}
	return newgroupid;
}
